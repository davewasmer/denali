...
export default class Builder {
  ...
  constructor(dir, project, preseededAddons) {
    ...
    this.lint = this.isRootBuilder ? project.lint : false;
  ...
  toTree() {
    ...
    // Dedup common addons - if it's the same underlying filesystem dir, no need
    // to create a separate builder for it.
    let projectBuilderDirs = this.project.builders.map((builder) => builder.dir);
    addons = addons.filter((addonDir) => !projectBuilderDirs.includes(addonDir));
    // Create a Builder for each unique addon on disk
    let addonBuilders = addons.map((addonDir) => new Builder(addonDir, this.project));
    // Register it with the project so we can check for dups later, as well as
    // iterate over all builders in case one has a processEach() hook.
    this.project.builders.push(...addonBuilders);
