...
export default class Builder {
  ...
  constructor(dir, project, preseededAddons) {
    ...
    this.project.builders.push(this);
  ...
  _createChildBuilders() {
    ...
    return addons.map((addonDir) => {
      let builder = this.project.builders.find((b) => b.dir === addonDir);
      if (!builder) {
        builder = new Builder(addonDir, this.project);
      }
      return builder;
    });
  }
  ...
  toTree() {
    if (!this._tree) {
      let sourceTree = this._prepareSelf();
      let childBuilders = this._createChildBuilders();
      ...
      let addonTrees = childBuilders.map((builder) => builder.toTree());
